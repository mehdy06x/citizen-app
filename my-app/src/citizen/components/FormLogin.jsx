    ...
    const handleSubmit = async (e) => {
        ...
        try{
            ...
            if (method === "login"){
                ...
                // after login, decide where to go by probing admin endpoint
                try{
                  const verify = await api.get('/api/reclamations/all/');
                  if (verify.status === 200) navigate('/admin');
                  else navigate('/make-report');
                }catch(err){
                  navigate('/secondpage');
                }
            }else {
